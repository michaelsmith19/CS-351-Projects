...
public class AgentManagerImpl implements AgentManager {
	...
	/**
	 * makes an AgentManger with a {@link ConcurrentHashMap<Agent,Grid>} to hold
	 * the agents currently in each matrix(Grid). It also holds another
	 * {@link ConcurrentHashMap<Agent, Connection>} to keep track of which
	 * incoming tags came from which agent.
	 */
	...
	public ArrayList<Agent> getAgentsInside(Grid g) {
		...
		for (Agent a : agentToGrid.keySet()) {
			if (g == agentToGrid.get(a)) {
				agentList.add(a);
				System.out.println("Agent: " + a.getObjid() + " Is now in : "
						+ g);
			}
		}
		return agentList;
