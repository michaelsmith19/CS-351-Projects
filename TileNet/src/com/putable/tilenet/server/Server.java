...
public class Server implements Callable<Void> {
	 ...
	 * {@link Controller} will place events into its {@link LinkedBlockingQueue}
	 * 
	 * @author ClassCastExceptions
	 */
	public final class ServerDispatch implements Callable<Void> {
		...
		private final ConcurrentMap<XMLTag, Connection> whoTheFuckSentThis;
		...
		public ServerDispatch() {
			...
			this.whoTheFuckSentThis = new ConcurrentHashMap<XMLTag, Connection>();
		...
		public Void call() throws Exception {
			...
			while (true) {
				command = queue.take().getTag();
				conn = whoTheFuckSentThis.get(command);
				whoTheFuckSentThis.remove(command);
				System.out.println("Dispatch executing command: " + command
						+ " From " + conn);
				switch (command.getTagType()) {
				case CLIENT:
					validateClientTag((ClientTag) command, conn);
					break;
				case CMD:
					break;
				case HEAR:
				...
				case LOGGEDOUT:
					break;
				case LOGIN:
					validatePasswd((LoginTag) command, conn);
					break;
				case LOGOUT:
					break;
				case SERVER:
		 ...
		 */
		public synchronized void requestServerTag(Controller conntroller,
				Connection connection) {
			/*
			 * Server
			 * 
			 * ((c8.8.4)) TAG PURPOSE: Announce a server's identifying
			 * information and status to a (potential) client.
			 * 
			 * ((c8.8.5)) TAG SEQUENCING: Is the document root tag for the
			 * server side of the communication, so obviously the server tag
			 * open must be the first thing sent after the XML and DTD headers,
			 * and the server tag close must be the last thing sent before the
			 * server output side of the connection is closed.
			 */
			XMLTag serverTag = serverStatus();
			connection.sendMessage(serverHeader + serverTag);
		}

		private void validateClientTag(ClientTag tag, Connection conn) {
			/*
			 * Client
			 * 
			 ...
			 * 
			 * ((c8.1.6.1.3)) SERVER: MUST ensure this value is present and
			 * correct (which is automatically done by parsing it with a
			 * validating XML parser) and treat the input as a parsing error if
			 * it isn't.
			 */
			if (tag.getVersion().compareTo("1.1") == 0) {
				// TODO Nothing??
			} else {
				System.err.println("Error in version number. Got: "
						+ tag.getVersion());
				conn.disconnect();
			}
		}

		private void validatePasswd(LoginTag command, Connection conn) {
			/*
			 * Login
			 * 
			 * ((c8.6.4)) TAG PURPOSE: Authenticate a client to a server and
			 * associate a particular user name and connection information with
			 * a particular agent instance in the world.
			 * 
			 * ((c8.6.5)) TAG SEQUENCING: Is sent at least once after the client
			 * tag is opened. After sending, client MUST wait for a logged-in
			 * tag from server before sending anything more. If the returned
			 * logged-in tag indicates success, client MUST send no further
			 * login tags during that session. If the returned logged-in tag
			 * indicates failure, client MAY choose to modify the user and/or
			 * password information (presumably in consultation with the
			 * client's user) and send another login tag.
			 */
			if (passwds.get(command.getUser()).compareTo(command.getPassword()) == 0) {
				/*
				 * Logged-in
				 * 
				 * ((c8.4.4)) TAG PURPOSE: Allow a server to let a client know
				 * whether the client's login attempt succeeded or failed. In
				 * addition, if the login attempt succeeded, inform the client
				 * of the identity of the agent associated with the client.
				 * 
				 * ((c8.4.5)) TAG SEQUENCING: A logged-in tag MUST be the first
				 * tag sent after a server open tag, sent upon receipt of a
				 * login tag from the client. Multiple logged-in tags may be
				 * sent in a single session but only when all but the last one
				 * specific failure to login.
				 */
				ElementBuilder makeElements = new TileNetElementBuilder();
				Agent jones = (Agent) makeElements.orderElement(ElemType.AGENT);

				conn.sendMessage(new LoggedIn(jones.getObjid(),
						"Welcome to TileNet " + command.getUser()
								+ ". You have been assigned #"
								+ jones.getObjid()));
				System.out.println("SENT: "
						+ new LoggedIn(jones.getObjid(), "Welcome to TileNet "
								+ command.getUser()
								+ ". You have been assigned #"
								+ jones.getObjid()));

				// ALL LOGGED IN :)
				state.giveAgent(jones, conn);
		 ...
		 * 
		 * @return an open or busy {@link ServerTag}
		 * 
		 */
		private XMLTag serverStatus() {
			return (whoTheFuckSentThis.size() >= maxConnections) ? new ServerTag(
		 ...
		 */
		public void put(Connection connection, XMLTag command) {
			whoTheFuckSentThis.put(command, connection);
			queue.add(new Pair(command, connection));
		}
		...
		private void terminateAllConnections() throws IOException {
			for (Connection conn : whoTheFuckSentThis.values()) {
				conn.disconnect();
			}
	...
	public int getConnected() {
		return dispatch.whoTheFuckSentThis.size();
	}
