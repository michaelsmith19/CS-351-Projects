...
import com.putable.tilenet.Util.XMLTags.LogoutTag;
...
import com.putable.tilenet.Util.XMLTags.XResponseTag;
 ...
 * This Client contains the specific {@link Agent} using this client,
 * The main display, the {@link ChatPanel} the {@link AgentsConnectedPanel}
 * and has a dispatcher that will handle tags received from the {@link WorldModelImpl}
 * as well as tag received from {@link Server}
 *
 */
@SuppressWarnings("serial")
public class TileNetClient extends JFrame {
	// TODO change the dtd file to the one in our resources folder.
	...
	private final AgentsConnectedPanel agentPanel;
	...
	public TileNetClient() {
		...
		chat = new ChatPanel();
		agentPanel = new AgentsConnectedPanel();
		logout = new JButton("logout");
		this.conn = new AgentConnection();
		this.bq = new LinkedBlockingQueue<Pair>();
	...
	private JPanel makeWelcomePanel() {
		...
		whereWeAre.setConnection(conn);
		panel.add(tmp, BorderLayout.NORTH);
		panel.add(agentPanel, BorderLayout.EAST);
		panel.add(chat, BorderLayout.SOUTH);
		panel.add(logout, BorderLayout.WEST);
		logout.addMouseListener(new MouseAdapter() {
			@Override
			public void mouseClicked(MouseEvent e) {
				LogoutTag logout = new LogoutTag("good bye");
				
				conn.sendMessage(logout);
			}
		});
		chat.getTextField().addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent arg0) {
				...
				// First transform the string into valid XML CDATA.
				cmdTag.setText(AttributeTransformer.stringToAttribute(chat
	...
	private boolean connect() {
		...
		try {
			...
			Thread parseThread = new Thread(xmlp, "parseThread TilenetClient");
			parseThread.start();
	...
	private boolean exchangeOpenTag() throws InterruptedException {
		...
		if (p.getTag().getTagType() != TagType.SERVER) {
			 ...
			 */
			conn.sendMessage(clientHeader);
			conn.sendMessage(new ClientTag());
			success = true;
		}
	...
	private boolean exchangeLoggedin() throws InterruptedException {
		...
		conn.sendMessage(new LoginTag(userName, passwd.getText()));
	...
	private void processSetTag(XMLTag xt)
	{
		SetTag st = (SetTag) xt;
		if(st.getObjid().charAt(0) == 'a')
		{
			Agent a = (Agent) Common.toElement(st);
			if (st.getX() != null && st.getX() < 0)
				agentPanel.removeAgent(a);
			else
			{		
				agentPanel.addAgent(a);
			}
		}
		else whereWeAre.update(st);
		...
		public Void call() throws Exception {
			...
			while (true) {
				...
				case HEAR:
					...
					// Use the transformer to make all the text into its
					// character representations.
					String correctString = AttributeTransformer
							.attributeToString(ht.getMessage());
					ht.setMessage(correctString);
				...
				case SET:
					processSetTag(tag);
					break;
				case XRESPONSE: System.out.println("received garbage:" + ((XResponseTag) tag).getOptions());
					break;
				default:
					System.err
