...
public class Server implements Callable<Void> {
	...
	public final class ServerDispatch implements Callable<Void> {
		...
		public Void call() throws Exception {
			...
			while (true) {
				...
				case CLIENT:
					...
					break;
				case CMD:
					CMDTag tag = (CMDTag) command.getTag();
					if (tag.getCMDType().equals("say"))
						state.processSayCMD((CMDTag) command.getTag(),
								currentlyLoggedIn.get(c));
					if (tag.getCMDType().equals("click")) {
						state.processClick((CMDTag) command.getTag(),
								currentlyLoggedIn.get(c));
					}
		...
		private void sendLoggedout(Pair command) {
			LoggedOut loggedOut = new LoggedOut("logging you out");
			command.getConnection().sendMessage(loggedOut);
			try {
				this.terminateConnection(command.getConnection());
			} catch (IOException e) {
		 ...
		 */
		public void requestServerTag(Controller conntroller,
		...
		private void validateClientTag(Pair p) {
			 ...
			 * it isn't.
			 */
			if (tag.getVersion().compareTo("1.0") == 0) {
		...
		private void validatePasswd(Pair p) {
			...
			// If it is the first time logging in add the username/pword to the
			// hashmap.
			if (!passwds.containsKey(tag.getUser())) {
				passwds.put(tag.getUser(), tag.getPassword());
			}
		...
		/**
		 * Put an event into the {@link LinkedBlockingQueue} method used by
		 * {@link Controller}
		 * 
		 * @param command
		 ...
		 * @throws IOException
		 */
		public void terminateConnection(Connection conn) throws IOException {
